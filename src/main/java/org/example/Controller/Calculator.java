...
import org.example.Model.CalcLog;
import org.example.Model.Calculable;
import org.example.Model.ImaginaryNumber;
...
public class Calculator implements ICalculator{
    ...
    public void run() throws IOException {
        while (true) {
            view.show(arg1);
            this.calc = new CalcLog(newImgNum());
            while (true) {
    ...
    private ImaginaryNumber newImgNum() {
        return new ImaginaryNumber(view.promptInt("Целое число - "),
                view.promptInt("Число с приставкой i - "));
    }
}
